...
2. Bot が「企画会議 / 反省会 / 市場調査」の選択肢を出すので、目的のプロセスを選ぶ

#### 反省会

1. 社長命令チャンネルに短い誘導が出るので、反省会チャンネル（`POST_MORTEM_CHANNEL_ID`）へ移動する
2. 反省会チャンネルで直近リンク済みゲームの提案を確認し、`はじめる` を押す
3. 同チャンネルに各 AI 社員（すずかちゃん / スゴ杉くん / ヂャイアン）名義で教訓 before/after が投稿される

※ `artifact_stem` の手入力は不要です（`spec_game_links.json` の直近リンクから自動解決）。

#### 市場調査

1. 社長命令チャンネルに短い誘導が出るので、市場調査チャンネル（`MARKET_RESEARCH_CHANNEL_ID`）へ移動する
2. 確認 UI で `はじめる` を押す
3. 完了後、同チャンネルにヂャイアン名義でトレンド／メカニクスの概要が報告される（詳細 JSON は `shared/research/`）

#### 企画会議

1. 社長命令チャンネルに短い誘導が出るので、企画検討チャンネル（`MEETING_CHANNEL_ID`）へ移動する
2. ヂャイアンが `latest_trends.json` × `mechanics_db.json` から 3〜4 件のテーマ案を提示する
3. 案を選ぶと会議開始。気に入らなければ `もう一度検討` / `フリー入力` / `中止` を選ぶ
4. 途中経過・Go/NoGo/中止ボタンは同チャンネルに出る

※ 調査 JSON が無い場合は会議を始めず、先に市場調査するよう案内します。
